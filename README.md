...
Automated social card rendering and publishing for **Chenson Wedderburn, REALTOR®** —
...
```
content/*.json      one record per post — the copy, the figures, the source
      ↓
...
render.html         one renderer, 13 templates, driven by the record
      ↓
render.js           headless Chrome screenshots it at 1080 × 1080 → out/*.png
      ↓
...
Instagram will not accept raw image bytes — it fetches from a URL you hand it.
...
| **Tue–Fri** | `publish.yml` posts the card whose filename matches that date |
...
node scripts/render.js                        # render everything → out/
...
on that app — which is Chenson on both sides.
...
notes — hard gates. Nothing renders if one fails.
...
- No bare quantifiers — *most, many, usually* are statistics wearing words.
...
Nothing stores a date for a particular year — it stores the rule, so future years compute
themselves.
...
**Titan** — husky-owl. Army, then real estate. Carries the substance and the figures.
Explains things because he once signed papers he did not understand and got lucky.

**Echo** — raccoon-owl. Not in the business, which is the point: he asks what everyone
else is too embarrassed to ask, then lands on what he would do about it.

Titan speaks, Echo checks it back. That exchange is not decoration — it is how the
